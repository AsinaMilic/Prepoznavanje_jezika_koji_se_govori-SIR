"""
CNN Model za klasifikaciju jezika iz spektrograma.
"""
...
class CNNLanguageClassifier:
    """
    Konvoluciona neuronska mreža za klasifikaciju jezika iz spektrograma.
    """
    
    def __init__(self, input_shape: Tuple[int, int, int], num_classes: int,
                 filters: list = None, dense_units: int = 128, dropout_rate: float = 0.5):
        """
        Inicijalizuje CNN klasifikator.
        
        Args:
            input_shape: Dimenzije ulaznog spektrograma (height, width, channels)
            num_classes: Broj jezika za klasifikaciju
            filters: Lista broja filtera po sloju (default: [32, 64, 128])
            dense_units: Broj neurona u dense sloju (default: 128)
            dropout_rate: Dropout rate (default: 0.5)
        """
        self.input_shape = input_shape
        self.num_classes = num_classes
        self.filters = filters if filters is not None else [32, 64, 128]
